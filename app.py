...
show_top5 = st.sidebar.toggle(
    "🔴 출동건수 상위 5개동",
    value=False,
    help="켜면 출동건수가 가장 많은 상위 5개 행정동을 강조합니다.",
)

show_bottom5 = st.sidebar.toggle(
    "🔵 출동건수 하위 5개동",
    value=False,
    help="켜면 출동건수가 가장 적은 하위 5개 행정동을 강조합니다.",
)


# 상위/하위 5개 행정동 계산
top5_codes = set(
    df.nlargest(5, "emergency_count")["ADM_CD"].astype(str)
)

bottom5_codes = set(
    df.nsmallest(5, "emergency_count")["ADM_CD"].astype(str)
)

selected_codes = set()

if show_top5:
    selected_codes.update(top5_codes)

if show_bottom5:
    selected_codes.update(bottom5_codes)


plot_df = df.copy()

# 두 토글 중 하나라도 켜져 있으면 선택된 행정동만 색상 표시
# 나머지 행정동은 흰색으로 표시
if selected_codes:
    # 선택된 행정동은 실제 출동건수,
    # 선택되지 않은 행정동은 별도 값으로 두어 회색 처리
    plot_df["display_count"] = plot_df["emergency_count"]
    plot_df["is_selected"] = (
        plot_df["ADM_CD"].astype(str).isin(selected_codes)
    )
else:
    plot_df["display_count"] = plot_df["emergency_count"]
    plot_df["is_selected"] = True
...
# 선택 토글이 켜진 경우:
# 선택 지역은 출동건수 색상, 나머지 지역은 회색으로 별도 표시
if selected_codes:
    selected_df = plot_df[plot_df["is_selected"]].copy()
    unselected_df = plot_df[~plot_df["is_selected"]].copy()

    # 기본 fig를 선택된 행정동만 대상으로 다시 생성
    fig = px.choropleth_map(
        selected_df,
        geojson=geojson,
        locations="ADM_CD",
    ...
    # 선택되지 않은 지역은 회색 polygon으로 추가
    gray_fig = px.choropleth_map(
        unselected_df,
        geojson=geojson,
        locations="ADM_CD",
        featureidkey="properties.ADM_CD",
    ...
    for trace in gray_fig.data:
        fig.add_trace(trace)

    # 회색 trace는 색상바에 영향을 주지 않도록 설정
    if len(fig.data) > 1:
...
# ============================================================
# 행정동 출동건수 + 소방시설 위치 통합 지도
# ============================================================
st.subheader("🗺️ 서울시 출동건수 및 소방시설 통합 지도")
...
if FACILITY_FILE is not None:
    try:
        ...
        # 첫 번째 지도(Choropleth)에 소방시설 marker trace를 추가
        for trace in facility_fig.data:
            fig.add_trace(trace)
        ...
        st.warning(
            "소방시설 위치 데이터를 지도에 추가하지 못했습니다."
        )
        st.exception(e)
else:
    st.info(
        "소방시설 Excel 파일이 없어 행정동 출동건수 지도만 표시합니다."
    )
...
st.caption(
    "※ 행정동 출동건수는 흰색→빨간색으로 표시되며, "
    "소방서·안전센터·구조대 위치가 같은 지도 위에 표시됩니다."
)
